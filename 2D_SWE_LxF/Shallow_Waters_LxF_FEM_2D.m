...
while t < T
    ...
    for ip = 1:mesh.NP
        ...
        for vtx = 1:4
            ...
            % Mean of Fx and Fy
            Fhx_mean(ip,1) = sum(Fhx(V))/4;
            FU1x_mean(ip,1) = sum(FU1x(V))/4;
            FU2x_mean(ip,1) = sum(FU2x(V))/4;
            
            Fhy_mean(ip,1) = sum(Fhy(V))/4;
            FU1y_mean(ip,1) = sum(FU1y(V))/4;
            FU2y_mean(ip,1) = sum(FU2y(V))/4;
...
Fh_x = Fhx_mean - dxh; Fh_y = Fhy_mean - dyh;
FU1_x = FU1x_mean - dxUx; FU1_y = FU1y_mean - dyUx;
FU2_x = FU2x_mean - dxUy; FU2_y = FU2y_mean - dyUy;
